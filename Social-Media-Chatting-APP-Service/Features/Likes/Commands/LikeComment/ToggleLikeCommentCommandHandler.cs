...
public class ToggleLikeCommentCommandHandler(IUnitOfWork unitOfWork)
    : IRequestHandler<ToggleLikeCommentCommand, Result<bool>>
{
    public async Task<Result<bool>> Handle(ToggleLikeCommentCommand request, CancellationToken cancellationToken)
    {
        ...
        var comment = await commentRepo.FindAsync(p => p.Id == request.CommentId && !p.IsDeleted);
        if (comment is null)
            return Error.NotFound("Post.NotFound", "Post not found");

        var existingLike =
            await unitOfWork.FindAsync<CommentLike>(pl =>
                pl.CommentId == request.CommentId && pl.UserId == request.UserId);
